...
def deaths_file_path(ssp_tag, adaptation_name):
    return DESKTOP / f"Projection_{ssp_tag}_Grid_Deaths_{adaptation_name}_2025-2080.csv"


SSP245_DEATHS = deaths_file_path("SSP245", "NoAdaptation")
...
def calculate_period_stats(
    df_long, pop_df, period_start, period_end, scenario, heat_adaptation
):
    ...
    grid_age_temp_stats["Heat_Adaptation"] = heat_adaptation
...
for adaptation_name in ADAPTATION_SCENARIOS:
    scenarios.append(
        (
            "SSP2-4.5",
            deaths_file_path("SSP245", adaptation_name),
            ssp2_pop,
            adaptation_name,
        )
    )
    scenarios.append(
        (
            "SSP5-8.5",
            deaths_file_path("SSP585", adaptation_name),
            ssp5_pop,
            adaptation_name,
        )
    )
...
for scenario_name, deaths_file, pop_df, heat_adaptation in scenarios:
