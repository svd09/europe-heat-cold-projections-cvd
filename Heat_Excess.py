"""
Calculate Heat Excess for Europe Grid Cells - OPTIMIZED VERSION
Baseline Period: 2016-2023
- TMREL = Modal temperature (1 d.p.) from 54th-92nd percentile
- Heat excess = Daily temperatures above TMREL
...
print("="*80)
print("CALCULATING HEAT EXCESS FOR EUROPE GRID CELLS")
...
if missing_before > 0:
    ...
    # Impute missing values using ULTRA-FAST merge-based approach
...
# ============================================================================
# CALCULATE HEAT EXCESS FOR EACH GRID CELL
# ============================================================================
...
for idx, grid_id in enumerate(unique_grids):
    ...
    # STEP 2: Calculate heat excess above TMREL
    grid_data_clean['heat_excess'] = grid_data_clean['temp_rounded'] - tmrel
    
    # Only count positive heat excess (temperatures above TMREL)
    grid_data_clean.loc[grid_data_clean['heat_excess'] <= 0, 'heat_excess'] = 0
    
    # Total heat excess across entire study period
    total_heat_excess = grid_data_clean['heat_excess'].sum()
    
    # STEP 3: Calculate average daily heat excess
    # Divide total heat excess by total study period days
...
print(f"\n{'='*80}")
print("RESULTS SUMMARY - HEAT EXCESS")
print("="*80)
...
print(f"{'Total Heat Excess (°C)':<40} {results_df['Total_Heat_Excess_Celsius'].mean():<12.2f} {results_df['Total_Heat_Excess_Celsius'].median():<12.2f} {results_df['Total_Heat_Excess_Celsius'].min():<12.2f} {results_df['Total_Heat_Excess_Celsius'].max():<12.2f}")
print(f"{'Avg Daily Heat Excess (°C)':<40} {results_df['Avg_Daily_Heat_Excess'].mean():<12.4f} {results_df['Avg_Daily_Heat_Excess'].median():<12.4f} {results_df['Avg_Daily_Heat_Excess'].min():<12.4f} {results_df['Avg_Daily_Heat_Excess'].max():<12.4f}")
print(f"{'% Days Above TMREL':<40} {results_df['Percent_Days_Above_TMREL'].mean():<12.2f} {results_df['Percent_Days_Above_TMREL'].median():<12.2f} {results_df['Percent_Days_Above_TMREL'].min():<12.2f} {results_df['Percent_Days_Above_TMREL'].max():<12.2f}")

print(f"\nSample of results (first 10 grid cells):")
print(results_df[['grid_id', 'TMREL', 'Total_Heat_Excess_Celsius', 
                  'Avg_Daily_Heat_Excess', 'Percent_Days_Above_TMREL']].head(10).to_string(index=False))
...
print(f"  • Avg_Daily_Heat_Excess - For RR calculation (°C)")
print(f"  • Total_Heat_Excess_Celsius - Sum of all daily heat excesses (°C)")

print(f"\nNEXT STEPS:")
print(f"  1. Run cold excess script")
print(f"  2. Merge heat + cold results")
print(f"  3. Calculate RR and PAF")
print("="*80)
