#!/usr/bin/env python3
"""
Calculate Future Cold Excess (2025-2080) for Europe Grid Cells
...
2. For each year (2025-2080):
   ...
   - For each grid:
     - Calculate daily cold excess: TMREL - temp (only when temp < TMREL)
     - Sum to annual cold excess
3. Output: grid_id, year, annual_cold_excess

ASSUMPTIONS:
- TMREL stays constant (no adaptation scenario)
- Cold excess = sum of daily excesses below TMREL
"""
...
print("="*80)
print("CALCULATING FUTURE COLD EXCESS (2025-2080)")
...
tmrel_file = baseline_path / "Europe_Grid_Heat_Excess_2016-2023.csv"  # Using heat file for TMREL (should be identical to cold file)
...
try:
    ...
    print(f"\n   Please run Cold_Excess.py first to generate baseline TMREL")
    exit()
...
for scenario_name, scenario_file in scenarios.items():
    ...
    for year in years_to_process:
        ...
        # ====================================================================
        # STEP 3: Calculate cold excess for each grid
        # ====================================================================
        
        print(f"  Calculating cold excess per grid...")
        ...
        print(f"\n  ✓ Calculated cold excess for {len(grid_results):,} grids")
    ...
    print(f"\nCold Excess Summary ({scenario_name}):")
    print(f"  Mean avg daily cold excess: {results_df['avg_daily_cold_excess'].mean():.4f}°C")
    print(f"  Median avg daily cold excess: {results_df['avg_daily_cold_excess'].median():.4f}°C")
    print(f"  Max avg daily cold excess: {results_df['avg_daily_cold_excess'].max():.4f}°C")
    
    # Show trend over time
    annual_avg = results_df.groupby('year')['avg_daily_cold_excess'].mean()
    print(f"\nTrend over time (Europe-wide average daily cold excess):")
